...
class MemoryBuffer(object):
    ...
    def memorize(self, state, action, reward, next_state):
        ...
        # save new transistion and give it a biggest priority
        if(self.with_per):
            max_p = np.max(self.buffer.tree[-self.buffer.capacity:])#return biggest priority
            if max_p == 0:
    ...
    def priority(self, error):
        ...
        """
        return (error + self.belta) ** self.alpha
    ...
    def sample_batch(self, batch_size):
        ...
        if(self.with_per):
            T = self.buffer.total() // batch_size #calculate sample segment
            for i in range(batch_size):
                a, b = T * i, T * (i + 1)
                s = random.uniform(a, b)
                idx, error, data = self.buffer.get(s)
                #print(type(idx), type(error), data)
                batch.append((*data, idx)) # * collects all the positional arguments in a tuple.
